...
def map_request_to_classes(requestJson: str) -> Schedule:
    ...
    weekStart = datetime(requestJson['MondayOfWeekToGenerateFor']['year'], requestJson['MondayOfWeekToGenerateFor']['month'] + 1, requestJson['MondayOfWeekToGenerateFor']['date'])
    ...
    for employee in requestJson['Employees']:
        ...
        for shift in employee['PastShifts']:
            currShiftStart = datetime(shift['startDate']['year'], shift['startDate']['month'] + 1, shift['startDate']['date'], shift['startTime']['hours'], shift['startTime']['minutes'])
            currShiftEnd = datetime(shift['endDate']['year'], shift['startDate']['month'] + 1, shift['endDate']['date'], shift['endTime']['hours'], shift['endTime']['minutes'])
